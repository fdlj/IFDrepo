...
app.use(express.static(path.join(__dirname, 'dist/IFD')));
app.use('/', express.static(path.join(__dirname, 'dist/IFD')));
app.use(jwt());
...
// }
      ...
      console.log('server.js   Database ',dbConfig,' sucessfully connected')
   },
   error => {
      console.log('server.js   Database ',dbConfig,'could not connected: ' + error)
   }
)
...
// STEP 1.1
const pebbleMaisRoute = require('../backend/routes/pebble-mais.route')
